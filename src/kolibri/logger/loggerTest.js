...
loggerSuite.add(
    'test debug tag formatted-with-stacktrace log message',
    (assert) =>
        withDebugTestArrayAppender((appender) => {
            ...
            assert.is(result, true)
            assert.is(
                appender.getValue()[0].includes('loggerTest.js:139'),
                true
            )
        })
)
    ...
    withDebugTestArrayAppender((appender) => {
        ...
        assert.is(
            appender
                .getValue()[0]
                .toString()
                .startsWith(`Error: cannot evaluate log message`),
            true
        )
    })
)
...
loggerSuite.add('test error in message formatting code', (assert) =>
    withDebugTestArrayAppender((appender) => {
        ...
        assert.is(
            appender
                .getValue()[0]
                .startsWith(`Error: cannot format log message`),
            true
        )
    })
)
...
loggerSuite.add(
    'test lazy evaluation, logger should not log and function should not be evaluated',
    (assert) =>
        withDebugTestArrayAppender((appender) => {
            setLoggingLevel(LOG_NOTHING)
            const debug = debugLogger(LOG_CONTEXT_KOLIBRI_TEST)
