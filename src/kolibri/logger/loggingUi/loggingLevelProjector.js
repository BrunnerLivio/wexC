...
const projectLoggingChoice = (loggingLevelController) => {
  ...
  `)
    ...
    // data binding
    loggingLevelController.onValueChanged(
        (levelStr) => (select.value = levelStr)
    )
    loggingLevelController.onLabelChanged(
        (labelStr) => (label.textContent = /** @type { String } */ labelStr)
    )

    // view binding
    select.onchange = (_event) => loggingLevelController.setValue(select.value)

    return /** @type { [HTMLLabelElement, HTMLSelectElement] } */ [
        label,
        select,
    ]
}
