...
const LoggingUiController = () => {
    ...
    onLoggingLevelChanged((level) =>
        loggingLevelController.setValue(toString(level))
    )
    ...
    loggingContextController.onValueChanged((contextStr) =>
        setLoggingContext(contextStr)
    )
    // domain binding: when the global logging context changes, we need to update the string of the logging context
    onLoggingContextChanged((context) =>
        loggingContextController.setValue(context)
    )
    ...
    const observableAppender = ObservableAppender(CountAppender())(
