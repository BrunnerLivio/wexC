...
import { toString, LOG_DEBUG } from '../logLevel.js'
import {
    getLoggingContext,
    getLoggingLevel,
    setLoggingContext,
...
logUiControllerSuite.add('test binding log level as text', (assert) => {
    ...
    assert.is(
        logUiController.loggingLevelController.getValue(),
        toString(oldLoggingLevel)
    )
...
logUiControllerSuite.add('test binding log context as text', (assert) => {
    ...
    assert.is(
        logUiController.loggingContextController.getValue(),
        oldLoggingContext
    )
