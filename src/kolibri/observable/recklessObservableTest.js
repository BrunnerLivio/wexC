...
observableSuite.add('edge case', (assert) => {
    ...
    obs.onChange((val, oldVal) => {
        // first subscriber changes obs value itself when notified: tricky edge case
    ...
    assert.is(
        ['start', 'start', 'start', 'second', 'second', 'second_x'].eq(log1),
        true
    )
...
observableSuite.add('memory leak warning', (assert) => {
    ...
    withDebugTestArrayAppender((appender) => {
        ...
        assert.is(
            appender.getValue()[0],
            'Beware of memory leak. 101 listeners.'
        )
    })
