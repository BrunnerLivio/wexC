...
asyncTest('asyncRelay set/get', (assert) => {
    ...
    scheduler.addOk((_) => {
        remoteObservableMap.getValue('a')((_) => assert.isTrue(false))((v) =>
            assert.is(v, valueA)
        )
    })
    ...
    scheduler.addOk((_) => {
        observableMap.getValue('a')((_) => assert.isTrue(false))((v) =>
            assert.is(v, valueB)
        )
    })
...
asyncTest('asyncRelay onChange remoteObservableMap', (assert) => {
    ...
    return new Promise((done) => {
        // change value through remoteObservableMap
        scheduler.addOk((_) => {
...
asyncTest(
    'asyncRelay observableMap set value three times from same work package',
    (assert) => {
        const observableMap = ObservableMap('observableMap')
        const remoteObservableMap = ObservableMap('remoteObservableMap')
...
asyncTest('asyncRelay many maps synced', (assert) => {
    ...
    testScheduler.addOk((_) => {
        // note: this doesn't need scheduler1 because we are added after the s1 tasks
        assert.is(romChanges.length, 1) // remoteObservableMap was asynchronously updated
        mapCount.times((n) => {
