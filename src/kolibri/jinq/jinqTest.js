...
jinqSuite.add('combine for pyth triples', (assert) => {
    const result = from(Range(2, Number.MAX_VALUE)) // infinite
        .combine((z) => Range(2, z))
...
jinqSuite.add('jinq with maybe', (assert) => {
    ...
    maybeName((_) => assert.isTrue(false))((n) => assert.is(n, 'Paul'))
})
...
jinqSuite.add('test pairWith with Nothing', (assert) => {
    ...
    // When
    const result = from(just2).pairWith(Nothing).result()
...
jinqSuite.add('json test', (assert) => {
 ...
 `)
  ...
  `)
    ...
    // When
    const outstandingHeroNames = from(JsonMonad(battleData))
        .select((x) => x['winner'])
