...
const { warn, info, debug } = LoggerFactory(
    'ch.fhnw.kolibri.navigation.siteController'
)
...
const SiteController = () => {
    ...
    const onUnsupportedUriHash = (handler) =>
        (unsupportedHashReaction = handler)

    const emptyPage = EmptyPage()
    const pageActivated =
        /** @type { IObservable<PageType> } */ Observable(emptyPage)
    const pagePassivated =
        /** @type { IObservable<PageType> } */ Observable(emptyPage)

    const allPages = {} // URI_HASH to Page
    const currentUriHash =
        /** @type { IObservable<UriHashType> } */ Observable(URI_HASH_EMPTY)
    ...
    window.onhashchange = () =>
        gotoUriHash(window.location.hash, /* direct */ false)
    ...
    const pageTransition = (newUriHash) => {
        if (currentUriHash.getValue() === newUriHash) {
            // guard
            return
        }
        info(
            `page transition from ${currentUriHash.getValue()} to ${newUriHash}`
        )
