...
const simpleInputProjectorSuite = TestSuite(
    'projector/simpleForm/simpleInputProjector'
)
...
simpleInputProjectorSuite.add('binding', (assert) => {
    ...
    assert.is(
        spanElement.getAttribute('data-id'),
...
asyncTest('projector/simpleForm/debounce (async)', (assert) => {
    ...
    const [_, spanElement] = projectDebounceInput(debounceTimeMs)(
        controller,
        'TEST'
    )
    const inputElement = spanElement.querySelector('input')
