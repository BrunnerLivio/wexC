...
import {
    timeStringToMinutes,
    totalMinutesToTimeString,
} from '../projectorUtils.js'
 ...
 */
const projectInput =
    (timeout) => (eventType) => (inputController, formCssClassName) => {
        if (!inputController) {
    ...
    `)
        ...
        /** @type {HTMLInputElement} */ const inputElement =
            spanElement.firstElementChild // ... we would use array deconstruction
        /** @type {HTMLSpanElement}  */ const [popoverElement] = select(
            spanElement,
            '[popover]'
        ) // the element that pops

        // view and data binding can depend on the type
        if (inputController.getType() === TIME) {
            ...
            inputElement.addEventListener(eventType, (_) =>
                inputController.setValue(
                    /** @type { * } */ timeStringToMinutes(inputElement.value)
                )
            )
            inputController.onValueChanged(
                (val) =>
                    (inputElement.value = totalMinutesToTimeString(
                        /** @type { * } */ val
                    ))
            )
        } else if (inputController.getType() === CHECKBOX) {
            // "checked" attribute vs boolean in model
            inputElement.addEventListener(eventType, (_) =>
                inputController.setValue(
                    /** @type { * } */ inputElement.checked
                )
            )
            inputController.onValueChanged(
                (val) => (inputElement.checked = /** @type { * } */ val)
            )
        } else {
            // standard binding for all remaining types, esp. TEXT and NUMBER.
            if (timeout !== 0) {
                // we need debounce behavior
                let timeoutId
            ...
            inputController.onValueChanged(
                (val) => (inputElement.value = /** @type { * } */ val)
            )
        }
        ...
        inputController.onNameChanged((name) =>
            inputElement.setAttribute('name', name || id)
        )
        inputController.onValidChanged((valid) =>
            inputElement.setCustomValidity(valid ? '' : 'invalid')
        )

        inputController.onEditableChanged((isEditable) =>
            isEditable
