...
addToTestingTable(testSuite)(
    createTestConfig({
        ...
        evalFn: (expected) => (actual) => {
            ...
            let result = false
            actual((_) => false)((x) => (result = x === unwrappedVal))
            return result
        },
...
testSuite.add(
    'test single element: should return the only element ',
    (assert) => {
        // Given
        ...
        // Then
        maybeMin((_) => assert.isTrue(false))((x) => assert.is(x, 1))
    }
)
...
testSuite.add('test empty sequence: should return Nothing', (assert) => {
    ...
    // Then
    maybeMin((_) => assert.isTrue(true))((_) => assert.isTrue(false))
})

testSuite.add(
    'test comparator on strings: should return the longest string',
    (assert) => {
        // Given
        const strings = ['a', 'aa']
        ...
        // Then
        maybeMin((_) => assert.isTrue(false))((x) => assert.is(x, 'a'))
    }
)
...
testSuite.add('test smallest element at the end of the iterable', (assert) => {
    ...
    // Then
    maybeMin((_) => assert.isTrue(false))((x) => assert.is(x, 0))
})

testSuite.add(
    'test smallest element at the start of the sequence',
    (assert) => {
        // Given
...
testSuite.add('test multiple of equal values', (assert) => {
    ...
    // Then
    maybeMin((_) => assert.isTrue(false))((x) => assert.is(x, 7))
})
