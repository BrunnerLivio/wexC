...
testSuite.add('test prototype: toString, show', (assert) => {
    ...
    withAppender(
        arrayAppender,
        LOG_CONTEXT_KOLIBRI_SEQUENCE,
        LOG_WARN
    )(() => {
        // noinspection JSCheckFunctionSignatures
        assert.is(range.toString(2), range.show(2))
        assert.is(
            arrayAppender.getValue()[0],
            'Sequence.toString() with maxValues might lead to type inspection issues. Use show(2) instead.'
        )
    })
})
...
testSuite.add('test prototype: pipe', (assert) => {
    ...
    const double = (x) => 2 * x
    // When
    const actual = Seq(1, 2).pipe(map(double), map(double))
    // Then
...
testSuite.add('operator tests', (assert) => {
    ...
    assert.iterableEq(
        Seq(1, 2, 0).dropWhere((x) => x > 1),
        Seq(1, 0)
    )
    assert.iterableEq(
        Seq(1, 2, 3).dropWhile((x) => x < 3),
        Seq(3)
    )
    assert.iterableEq(
        Seq(1, 2).map((x) => x * 2),
    ...
    assert.iterableEq(
        Seq(1, 3, 2).takeWhere((x) => x < 3),
        Seq(1, 2)
    )
    assert.iterableEq(
        Seq(0, 1, 2, 0).takeWhile((x) => x < 2),
        Seq(0, 1)
    )
    assert.iterableEq(
        Seq(1, 2)
...
testSuite.add('test prototype: terminal operations', (assert) => {
    assert.is(
        Seq(1, 2, 3).foldr$((acc, cur) => '' + acc + cur, ''),
        '321'
    )
    assert.is(
        Seq(1, 2, 3).foldl$((acc, cur) => '' + acc + cur, ''),
        '123'
    )
    assert.is(
        Seq(1, 2, 3).reduce$((acc, cur) => '' + acc + cur, ''),
        '123'
    )
    ...
    let x = ''
    Seq('x', 'xx').safeMax$((a, b) => a.length < b.length)(
        (_) => (x = 'error')
    )((n) => (x = n))
    assert.is(x, 'xx')

    assert.is(
        Seq('x', 'xxx', 'xx').min$((a, b) => a.length < b.length),
        'x'
    )

    let y = undefined
    Seq(3, 1, 2).safeMin$()((_) => (y = -1))((n) => (y = n))
    assert.is(y, 1)
