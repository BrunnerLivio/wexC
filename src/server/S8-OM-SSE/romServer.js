...
const handleValueUpdate = (req, res) => {
    ...
    if (req.method === 'POST') {
        ...
        req.on('end', (input) => {
            ...
            rom.getValue(key)(
                // set value only if version is higher than old version and data has really changed
                (_nothing) => {
                ...
                if (newVersion <= oldVersion) {
                    log.debug(
                        (_) =>
                            `new version ${newVersion} <= old version ${oldVersion} - not setting key ${key}`
                    )
                    return
                }
                const oldData = oldContent[KEY_DATA]
                const newData = contentToBeStored[KEY_DATA]
                if (oldData === newData || ownPropEqual(oldData, newData)) {
                    // todo: nested objects (we dont have them atm)
                    log.debug(
                        (_) =>
                            `version ${newVersion} is new but data did not change - not setting key ${key}`
                    )
                } else {
