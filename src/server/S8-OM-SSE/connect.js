...
const connect = (baseUrl, om) => {
        ...
        switch (action) {
            case PATH_UPDATE_ACTION:
                ...
                if (receivedVersion <= versions[key]) {
                    log.debug(
                        `ignore update: new version < old version:  ${receivedVersion} <= ${versions[key]}.`
                    )
                    return
                }
                if (receivedVersion === versions[key]) {
                    log.debug(
                        'version is equal but value might still have changed (?)'
                    )
                }
                scheduler.addOk((_) => {
                    let dataValue = value[KEY_DATA]
                    if (!(dataValue instanceof Object)) {
                        // todo remove dupl with obsMap
    ...
    rom.onChange((key, value) => {
        // also handles the keyAdded case implicitly
        if (value[remoteSymbol]) {
            // todo: think about enforcing immutable values for all obsMaps
            log.debug(
                `prevent echo of our own changes back to the server, key: ${key}`
            )
            return
        }
        ...
        sendingScheduler.add((done) => {
            log.debug(
                `sending update key ${key} value ${value} version ${versions[key]}`
            )
            client(baseUrl + '/' + PATH_UPDATE_ACTION, 'POST', data)
                .then((_) => log.debug('done sending update'))
                .catch((e) =>
                    log.error(
                        'error sending update data: ' +
                            JSON.stringify(data) +
                            ' ' +
                            e
                    )
