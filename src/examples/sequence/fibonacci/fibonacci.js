...
const fibonacciView = (rootElement, amount, scaleFactor) => {
    ...
    const values = _.zipWith((a, b) => ({ current: a, pos: b }))(fibSequence)(
        position
    )
    const indexed = _.zipWith((a, b) => ({ ...a, index: b }))(values)(
        Range(100)
    )
    const colored = _.zipWith((a, b) => ({ ...a, color: b }))(indexed)(
        AngleSequence(amount)
    )

    const elements = sequenceProjector(colored)(
        fibonacciProjector(scaleFactor)
    ).children

    rootElement.append(...elements)
}

const fibonacciProjector =
    (scaleFactor) =>
    ({ index, current, pos, color }) => {
