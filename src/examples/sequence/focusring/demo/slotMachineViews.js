...
const SlotMachineView = (rootElement, controller) => {
  ...
  `)
    ...
    const renderWheel = (wheel) => wheelProjector(wheelsElement, wheel)
...
/**
 * Draws the wheels to the UI. Each slot of the wheel is represented by a value of the {@link FocusRingType}.
 *
 * @param { HTMLElement }                rootElement
...
const wheelProjector = (rootElement, wheelObservable) => {
    ...
    const createSlot = (content) => dom(`<div class="slot">${content}</div>`)[0]
    ...
    wheelObservable.onChange((focusRing) => {
        ...
        wheelElement.removeChild(
            wheelElement.children[wheelElement.children.length - 1]
        )
    })
    ...
    document.addEventListener('mousemove', (evt) => {
        if (knobIsPulled) {
            ...
            if (evt.clientY < LEVER_MIDDLE_Y) {
                ...
                if (0 < dy) {
                    // only adjust lever size when moving downwards
                ...
                if (2 * LEVER_HEIGHT < dy) {
                    // only activate if lever was pulled all down to the bottom
                    activated = true
                } else {
