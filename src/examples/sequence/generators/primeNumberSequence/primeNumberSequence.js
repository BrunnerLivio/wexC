...
const PrimeNumberSequence = () => {
    ...
    const primeNumberIterator = () => {
        ...
        const next = () => {
            ...
            return maybePrime((_) => {
                const prevIterable = createMonadicSequence(
                    () => prevPrimesIterator
                ) // conserve the iterator state of the previous prime
                const nextPrime = patternForPrime(nextValue)
                const prevPrimes = zipWith(uncurry(choiceMaybe))(prevIterable)(
                    nextPrime
                )
                ...
                next
            ) // Just, at least one number found, which divides the current number
        }
