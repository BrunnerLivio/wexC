...
const FizzBuzzView = (controller, rootElement) => {
    ...
    const renderResult = (result) =>
        resultRoot.replaceChildren(resultProjector(controller, result))
...
const boundaryProjector = (controller) => {
    ...
    const [upperInput] = dom(
        `<input type="number" value="${controller.getUpperBoundary()}" id="upper" class="numberInput">`
    )
    const [lowerInput] = dom(
        `<input type="number" value="${controller.getLowerBoundary()}" id="lower" class="numberInput">`
    )

    upperInput.onchange = (_) =>
        controller.setUpperBoundary(Number(upperInput.value))
    lowerInput.onchange = (_) =>
        controller.setLowerBoundary(Number(lowerInput.value))
...
const resultProjector = (controller, result) => {
    const [numberedList] = dom(
        `<ol start="${controller.getLowerBoundary()}"></ol>`
    )
    numberedList.append(
        ...sequenceProjector(result)((row) => dom(`<li>${row}</li>`)[0])
            .children
    )
    return numberedList
}
...
const ruleProjector = (rule) => {
    ...
    container.append(ruleNrProjector(rule), ruleTextProjector(rule))
    return container
}
