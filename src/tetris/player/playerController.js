...
const PLAYER_ACTIVE_ID =
    /** @type { ForeignKeyType } */ PREFIX_IMMORTAL + 'PLAYER_ACTIVE_ID'
...
const PlayerController = (om, omPublishStrategy, onSetupFinished) => {
    ...
    const publish = (player) =>
        omPublishStrategy((_) => om.setValue(player.id, player))
    ...
    const publishRemoveKey = (playerId) =>
        omPublishStrategy((_) => om.removeKey(playerId))
    ...
    const handlePlayerUpdate = (player) => {
        const knownPlayerIndex = knownPlayersBackingList.findIndex(
            (it) => it.id === player.id
        )
        if (knownPlayerIndex >= 0) {
            ...
            if (player.id === PLAYER_SELF_ID) {
                // we are now known, which means the setup has finished
                onSetupFinished()
            }
        }
    ...
    const onWeHaveBecomeActive = (callback) => {
        activePlayerIdObs.onChange((playerId) => {
            if (playerId === PLAYER_SELF_ID) {
                // we have become in charge
                callback()
            }
        })
    }
    ...
    const leave = () => {
        ...
        if (areWeInCharge()) {
            // if we are in charge while leaving, put someone else in charge
            let nextCandidate = knownPlayersBackingList.at(0)
            if (thisIsUs(nextCandidate)) {
                // if that is ourselves, try the next one
                nextCandidate = knownPlayersBackingList.at(1)
            }
            publishReferrer(
                PLAYER_ACTIVE_ID,
                nextCandidate?.id ?? MISSING_FOREIGN_KEY
            )
        }
    }
    ...
    const startListening = () => {
        om.onKeyRemoved((key) => {
            if (key.startsWith(PLAYER_PREFIX)) {
                const player = knownPlayersBackingList.find(
                    (it) => it.id === key
                )
