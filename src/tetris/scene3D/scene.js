...
const registerForMouseAndTouch = (scene, consumeEvents = false) => {
    ...
    if (!coords) {
        throw new Error(
            `Element ${scene} must have a child with class "coords"`
        )
    }
    ...
    const track = (evt) => {
        ...
        const lastCoordXrotate = window
            .getComputedStyle(coords, null)
            .getPropertyValue('--coords-rotate-x')
        const lastCoordYrotate = window
            .getComputedStyle(coords, null)
            .getPropertyValue('--coords-rotate-y')
        coords.style.setProperty(
            '--coords-rotate-x',
            String(Number(lastCoordXrotate) - diffVertical)
        )
        coords.style.setProperty(
            '--coords-rotate-y',
            String(Number(lastCoordYrotate) + diffHorizontal)
        )
    }
    const consume = (evt) => {
        // prevent click, focus, drag, and selection events
        if (!consumeEvents) return
        evt.preventDefault()
        evt.stopImmediatePropagation()
    }
    scene.onmousedown = (evt) => {
        // start updating
        consume(evt)
        firstCall = true
