...
/**
 * @constructor
 * @param { import("../../kolibri/observable/observableMap.js").ObservableMapType } om
...
const GameController = (om) => {
    ...
    const checkAndHandleFullLevel = () => {
        ...
        const isFull = (level) =>
            boxController.findAllBoxesWhere((box) => box.zPos === level)
                .length >=
            7 * 7
    ...
    const onCollision = (tetromino) => {
        if (!playerController.areWeInCharge()) {
            // only the player in charge handles collisions
            return
        }
        if (tetromino.zPos > 11) {
            // we collide at the very top => end of game
        ...
        if (!playerController.areWeInCharge()) {
            // box removal is async and things might have changed.
            return
        }
     ...
     * @param { import("../shape/shapeController.js").NewShapeType } turnFunction
     */
    const turnShape = (turnFunction) => {
        ...
        const position = {
            xPos: currentTetromino.xPos,
            yPos: currentTetromino.yPos,
            zPos: currentTetromino.zPos,
        }

        const nextBoxPositions = tetrominoController.expectedBoxPositions(
            newShape,
            position
        )
     ...
     * @collaborator current tetromino and spaceBoxes
     * @impure everything might change.
     * @param { import("../shape/shapeController.js").NewPositionType } moveFunction
     */
    const movePosition = (moveFunction) => {
        ...
        const { x, y, z } = moveFunction({
            x: currentTetromino.xPos,
            y: currentTetromino.yPos,
            z: currentTetromino.zPos,
        })
    ...
    const isDisallowedTetroPosition = (nextBoxPositions) => {
        return nextBoxPositions.some((boxPos) =>
            isDisallowedBoxPosition(boxPos)
        )
    }
    ...
    const willCollide = (nextBoxPositions, tetromino) => {
        ...
        const otherBoxes = boxController.findAllBoxesWhere(
            (box) => box.tetroId !== tetromino.id
        )
    ...
    const registerKeyListener = () => {
        document.onkeydown = (keyEvt) => {
            // note: must be on document since not all elements listen for keydown
    ...
    const tetrominoController = TetrominoController(
        om,
        omPublishStrategy,
        boxController
    )
