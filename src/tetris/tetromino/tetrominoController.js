...
export { TetrominoController, TETROMINO_PREFIX, TETROMINO_CURRENT_ID }

const log = LoggerFactory('ch.fhnw.tetris.tetromino.tetrominoController')

const TETROMINO_PREFIX = 'TETROMINO-'
const TETROMINO_CURRENT_ID =
    /** @type { ForeignKeyType } */ PREFIX_IMMORTAL + 'TETROMINO_CURRENT_ID' // will never be removed once created
...
const TetrominoController = (
    ...
    const getCurrentTetrominoFromObservableMap = (id) => {
        ...
        if (NO_TETROMINO.id === id) {
            return result
        }
        observableMap.getValue(id)((_) =>
            log.warn('no tetromino with current id ' + id)
        )((tetro) => (result = tetro))
        return result
    }
    tetrominoCurrentIdObs.onChange((newId) =>
        currentTetrominoObs.setValue(
            getCurrentTetrominoFromObservableMap(newId)
        )
    )
    ...
    const startListening = () => {
        observableMap.onKeyRemoved((key) => {
            if (key.startsWith(TETROMINO_PREFIX)) {
                const tetromino = tetrominoBackingList.find(
                    (it) => it.id === key
                )
    ...
    const handleTetrominoUpdate = (tetromino) => {
        const knownTetroIndex = tetrominoBackingList.findIndex(
            (it) => it.id === tetromino.id
        )
    ...
    const expectedBoxPositions = (shape, position) => {
        ...
        return [0, 1, 2, 3].map((shapeIndex) =>
            finalBoxPosition(shadowTetromino, shapeIndex)
        )
    }
    ...
    const makeNewCurrentTetromino = () => {
        ...
        const tetroId =
            /** @type { ForeignKeyType } */ TETROMINO_PREFIX +
            clientId +
            '-' +
            runningNum++
    ...
    const publishUpdatedBoxPositions = (tetromino) => {
        ;[0, 1, 2, 3].map((boxIndex) => {
            ...
            const boxId =
                /** @type { ForeignKeyType } */ BOX_PREFIX +
                tetroId +
                '-' +
                boxIndex
