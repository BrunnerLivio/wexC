...
export { GameStateController, GAME_STATE }

const log = LoggerFactory('ch.fhnw.tetris.gameState.gameStateController')

const GAME_STATE =
    /** @type { ForeignKeyType } */ PREFIX_IMMORTAL + 'GAME_STATE'
...
const GameStateController = (om, omPublishStrategy) => {
     ...
     */
    const publish = (gameState) =>
        omPublishStrategy((_) => om.setValue(gameState.id, gameState))
    ...
    const updateScore = (changeFn) => {
        ...
        const newGameState = /** @type { GameStateModelType } */ {
            ...oldGameState,
        }
    ...
    const setFallingDown = (newValue) => {
        ...
        if (oldGameState.fallingDown === newValue) {
